...
test("cli: auth --set-key stores key without OAuth", () => {
  ...
  try {
    ...
    // Pre-seed config with orgId/defaultProject to ensure they get cleared.
    fs.writeFileSync(
      path.join(postgresaiDir, "config.json"),
      JSON.stringify({ orgId: 123, defaultProject: "old_project" }, null, 2) + "\n"
    );
    ...
    assert.ok(!("orgId" in config), "orgId should be removed on --set-key");
    assert.ok(!("defaultProject" in config), "defaultProject should be removed on --set-key");
  } finally {
    // Cleanup
