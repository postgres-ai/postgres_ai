...
class PostgresReportGenerator:
    ...
    def generate_i001_io_statistics_report(self, cluster: str = "local", node_name: str = "node-01") -> Dict[str, Any]:
        ...
        version_info = self._get_postgres_version_info(cluster, node_name)
        major_ver = version_info.get("server_major_ver", "0")
        try:
            pg_major = int(major_ver)
        except ValueError:
            pg_major = 0

        # pg_stat_io requires PostgreSQL 16+
        if pg_major < 16:
            logger.warning(f"I001 - pg_stat_io requires PostgreSQL 16+, found version {major_ver}")
            return self.format_report_data(
                "I001",
                {
                    "available": False,
        ...
        # Query pg_stat_io metrics using last_over_time
            ...
            if result.get('status') == 'success' and result.get('data', {}).get('result'):
                for item in result['data']['result']:
                    ...
                    if metric_name == "stats_reset_s":
                        # Use the max stats_reset_s value across all backend types
        ...
        read_hit_ratio_pct = round((total_hits / total_requests * 100), 2) if total_requests > 0 else 0
